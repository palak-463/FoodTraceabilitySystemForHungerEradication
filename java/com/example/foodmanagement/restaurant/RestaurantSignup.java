    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        register.setOnClickListener(new View.OnClickListener() {
            public void onClick(View view) {
                ...
                mAuth.createUserWithEmailAndPassword(e,pass).addOnCompleteListener(RestaurantSignup.this, new OnCompleteListener<AuthResult>() {
                    @Override
                    public void onComplete(@NonNull Task<AuthResult> task) {
                        if (task.isSuccessful())
                        {
                            FarmerRegister fm=new FarmerRegister(n,a,p,e,pass);
                            FirebaseDatabase.getInstance().getReference("RestaurantInfo")
                                ...
                                public void onComplete(@NonNull Task<Void> task) {
                                    if(task.isSuccessful())
                                    {
                                        Toast.makeText(RestaurantSignup.this, "Registered Successfully!", Toast.LENGTH_SHORT).show();
                                        Intent intent = new Intent(RestaurantSignup.this, RestaurantLogin.class);
                                        startActivity(intent);
                                    }
                                    else
                                    {
                                        Toast.makeText(RestaurantSignup.this, "Failed to register!", Toast.LENGTH_SHORT).show();
                                    }
                                }
